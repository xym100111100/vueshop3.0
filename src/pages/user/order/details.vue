<template>
  <div class="page">
    ...
    <div class="main">
      <div class="ordernum">订单编号：{{orderDesc.ordernum}}</div>
      ...
      <div class="goods-list" v-for="(item,index) in orderDesc.goods" :key="index">
        <div class="image">
      ...
      <ul class="order-status">
        <li>支付状态</li>
        <li>{{orderDesc.status ==="0"?"待付款":"已付款"}}</li>
      </ul>
      <div class="total-wrap">
        <ul class="total">
          <li>商品总额</li>
          <li>¥{{ orderDesc.total}}</li>
        </ul>
        <ul class="total">
          <li>+运费</li>
          <li>¥{{ orderDesc.freight}}</li>
        </ul>
      </div>
      <div class="true-total">
        <div class="total">
          实付金额：
          <span>¥{{ orderDesc.truetotal }}</span>
        </div>
        <div class="order-time">下单时间：{{ orderDesc.ordertime}}</div>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...mapState({
      orderDesc: state => state.order.orderDesc
    })
  },
  created() {
    this.getOrderDesc({ ordernum: this.ordernum });
  },
  methods: {
    ...mapActions({
